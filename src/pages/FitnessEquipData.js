export const equipmentDetails = [
    { name: "lat-pull-down",
        displayName: "랫 풀 다운",
        muscle: "등",
        description: "광배, 특히 등 상부 근육을 타겟으로 하여 세밀한 근육 발달을 위해 효과적이며,\n초보자부터 전문가까지 손쉽게 사용할 수 있습니다.\n자세 교정과 근력 향상에 도움이 됩니다.",
        exerciseInstructions: "1. 머신에 앉을 때 상체를 위치한 엉덩이보다 살짝만 뒤로 기울여 눕혀줍니다.\n2. 등을 펼 때 어깨에 힘을 주지 않고 자연스럽게 내림과 동시에 가슴을 내밉니다.\n3. 어깨너비보다 조금 넓게 그립을 잡고, 팔꿈치가 당겨오는 줄의 방향과 일진선이 되도록 바가 쇄골에 닿을 때까지 호흡을 내쉬며 당겨줍니다.\n4. 호흡을 들이마시며 시작 자세로 돌아옵니다.",
        warning:"1. 승모근에 힘을 주지 않고 아래로 떨어뜨린다고 생각하며 시작해야 합니다.\n2. 당겨올 때(수축할 때) 호흡을 들이마시면 어깨가 올라가 승모근에 긴장이 갈 수 있으니 주의해야 합니다.\n3. 팔을 완전히 펴면 어깨에 무리가 갈 수 있으니 주의해야 합니다.\n4. 그립을 잡는 손의 위치는 사람마다 다르므로 자극이 잘 오는 위치를 찾는 것도 좋습니다.",
        videos: [
            { 
                title: "랫 풀 다운 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/MpVD4WMoewM/0.jpg", 
                videoLink: "https://youtu.be/MpVD4WMoewM?si=skzTuLV39comcAPp"
            },
            { 
                title: "랫 풀 다운 기본 운동법2", 
                thumbnail: "https://img.youtube.com/vi/OJevd3OyNNo/0.jpg", 
                videoLink: "https://youtu.be/OJevd3OyNNo?si=lPHbPa2pYv17W_tW&t=30"
            },
            {
                title: "랫 풀 다운 기본 운동법3",
                thumbnail: "https://img.youtube.com/vi/uucD6XSsL-k/0.jpg",
                videoLink: "https://youtube.com/shorts/uucD6XSsL-k?si=WXI69950dm0SbYUH"
            },
            {
                title: "랫 풀 다운 그립 별 자극부위",
                thumbnail: "https://img.youtube.com/vi/E7aOkectaw4/0.jpg",
                videoLink: "https://youtube.com/shorts/E7aOkectaw4?si=2ZecPWUdo8Z2IhPg"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/lat-pull-down.jpeg"
    },
    { name: "cable-arm-pulldown",
        displayName: "케이블 암 풀 다운",
        muscle: "등",
        description: "삼두근과 광배를 타겟으로 하여 효과적으로 발달시킬 수 있습니다.\n팔을 올렸다 내리는 동작간의 부드러운 전환으로 관절에 부담을 덜 주고,\n자연스러운 활동 범위 내에서 신체를 움직이게 하여\n초보자들이 접근하기에 쉬운 운동입니다.",
        exerciseInstructions: "1. 다리를 어깨 너비만큼 벌립니다.\n2. 그립을 팔꿈치를 살짝 굽힌 상태로 잡습니다.\n3. 무릎을 살짝 굽히고 허리는 곧게 펴 45도 정도로 상체를 기울입니다.\n4. 호흡을 내쉬면서 반원을 그리듯 바를 허벅지 쪽으로 끌어내립니다.\n5. 호흡을 마시며 원위치로 돌아갑니다.",
        warning: "1. 승모근에 힘이 들어가지 않도록 어깨는 아래로 끌어내리는 동시에 가슴은 위로 업시키듯 내밀어야 합니다.\n2. 광배근에 긴장을 유지시키며 천천히 이완시켜야 합니다.\n3. 수축과 이완 동작 시 팔꿈치는 살짝 구부린 상태로 동작을 진행해야 하며, 상체는 흔들리지 않아야 합니다.",
        videos: [
            { 
                title: "케이블 스트레이트 암 풀 다운 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/C1_Yx8qPXRE/0.jpg", 
                videoLink: "https://youtu.be/C1_Yx8qPXRE?si=rFvxOyPfOXr2uAZR"
            },
            { 
                title: "케이블 스트레이트 암 풀 다운 기본 운동법2", 
                thumbnail: "https://img.youtube.com/vi/Jdyy0GNx4sc/0.jpg", 
                videoLink: "https://youtu.be/Jdyy0GNx4sc?si=8Z2fmbI2q5rMCAxo"
            },
            {
                title: "케이블 스트레이트 암 풀 다운 기본 운동법3", 
                thumbnail: "https://img.youtube.com/vi/YntwjLKHlLk/0.jpg", 
                videoLink: "https://youtu.be/YntwjLKHlLk?si=Tpzt3Z38GCTC72dm&t=15"
            },
            {
                title: "케이블 스트레이트 암 풀 다운 기본 운동법4", 
                thumbnail: "https://img.youtube.com/vi/cTckQ2sXQPM/0.jpg", 
                videoLink: "https://youtube.com/shorts/cTckQ2sXQPM?si=or77aF2bP3Fg28TN"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/cable-arm-pulldown.jpeg"
    },
    { 
        displayName: "체스트 프레스",
        name: "chest-press",
        muscle: "상체",
        description: "가슴 근육을 안정적으로 발달시키는 데 도움이 되며,\n타 가슴 운동에 비해 관절에 부담을 더 적게 주어\n초보자분들도 쉽게 단련할 수 있습니다.",
        exerciseInstructions: "1. 가슴을 내민다고 생각하며 허리를 세워 앉습니다.\n2. 두 손으로 그립을 잡아주고 허리를 살짝 아치 형태로 만들어주어 의자에 등이 닿지 않도록 합니다.\n3. 호흡을 내시며 팔꿈치가 완전히 펴지기 전까지 힘껏 밀어줍니다.\n4. 호흡을 들이쉬며 팔의 각도가 수직이 될 때까지만 이완시킵니다.",
        warning:"1. 팔꿈치를 완전히 펴면 무리가 갈 수 있으니 끝까지 펴지 않습니다.\n2. 수축 후 그립을 잡은 손을 일직선 방향으로 끌고와야 합니다.",
        videos: [
            { 
                title: "체스트 프레스 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/CAt37ltbjTI/0.jpg", 
                videoLink: "https://youtu.be/CAt37ltbjTI?si=_U98ROx49DW7i_YQ&t=419"
            },
            { 
                title: "체스트 프레스 기본 운동법2", 
                thumbnail: "https://img.youtube.com/vi/UqlUZFZgczk/0.jpg", 
                videoLink: "https://youtu.be/UqlUZFZgczk?si=3c2ftg1sZreDSRva&t=475"
            },
            { 
                title: "체스트 프레스 기본 운동법3", 
                thumbnail: "https://img.youtube.com/vi/eLbsMnfoCq8/0.jpg", 
                videoLink: "https://youtube.com/shorts/eLbsMnfoCq8?si=X3VLPykMP2yfqfZT"
            },
            { 
                title: "체스트 프레스 기본 운동법4", 
                thumbnail: "https://img.youtube.com/vi/AKzdQPAEGMQ/0.jpg", 
                videoLink: "https://youtube.com/shorts/AKzdQPAEGMQ?si=Z24coSqSdbYl2V6i"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/chest-press.jpeg"
    },
    { 
        displayName: "백 익스텐션",
        name: "back-extension",
        muscle: "등",
        description: "백 익스텐션은 주로 척추기립근을 대상으로 하는 운동으로,\n이 운동은 하부 등 근육과 코어 힘을 강화하는데 탁월하며,\n다른 허리 등 운동보다 코어 근육의 안정성을 강화하고\n자세를 개선하는 데에도 많은 도움이 됩니다.",
        exerciseInstructions: "1. 허벅지 받침의 높이를 편한 위치에 놓습니다.\n2. 다리는 발 받침 위에 어깨너비로 놓으며 종아리 아래쪽을 다리걸이에 걸어 고정시킵니다.\n3. 팔은 허리 뒤에 올려놓거나 양팔을 교차해 어깨에 위치시킵니다.\n4. 허리와 가슴을 펴고 턱을 당긴 자세로 호흡을 내쉬며 천천히 허리를 숙여 상체를 아래로 내립니다.\n5. 호흡을 마시며 코어 힘으로 상체를 올려줍니다.",
        warning: "1. 상체를 들어올릴 때 허리 힘으로 들어올리는 것이 아닌 코어 힘으로 들어올려 주어야 합니다.\n2. 허리가 과신전되지 않는 위치까지만 상체를 들어올려줍니다.\n3. 허리가 말리지 않도록 가슴을 곧게 펴줍니다.",
        videos: [
            { 
                title: "백 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/3vKhZRSNm0Y/0.jpg", 
                videoLink: "https://youtu.be/3vKhZRSNm0Y?si=yR4Lz-BwTp22ptup&t=602"
            },
            { 
                title: "백 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/u6hMWk8zD_4/0.jpg", 
                videoLink: "https://youtu.be/u6hMWk8zD_4?si=9xUpFUbWpANuTg2O&t=57"
            },
            { 
                title: "백 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/JekuOb9yctw/0.jpg", 
                videoLink: "https://youtube.com/shorts/JekuOb9yctw?si=J_1JkdKnUK-TuNYg"
            },
            { 
                title: "백 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/j8jm32Ej1Hk/0.jpg", 
                videoLink: "https://youtube.com/shorts/j8jm32Ej1Hk?si=BEQmSqX1BMa4tW-T"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/back-extension.jpeg"
    },
    { 
        displayName: "리버스 하이퍼(백) 익스텐션",
        name: "reverse-hyper-extension",
        muscle: "등",
        description: "허리와 엉덩이 근육을 강화하는 데 도움을 주며,\n상체를 고정시킨 상태에서 골반과 허리를 움직이므로\n허리의 과신전에 관해서 안전합니다.\n코어힘이 필요하기 떄문에 하체 근력 강화와 더불어\n상체의 안정성도 향상시키는 데 효율적인 운동입니다.",
        exerciseInstructions: "1. 발목 패드를 발목보다 살짝 위쪽에 위치하도록 각도를 맞춰줍니다.\n2. 손잡이를 잡고 패드 아래쪽에 고관절 뼈가 맞닿도록 엎드려 누워줍니다.\n3. 발목 패드에 두 다리를 댄 상태에서 호흡을 내쉬며 기립근과 둔근을 이용하여 다리를 들어 올려줍니다.\n4. 호흡을 들이쉬며 다리를 내려줍니다.",
        warning: "1. 다리를 들어올릴 때 허리가 과신전되지 않도록 합니다.\n2. 패드를 던지는 느낌으로 힘차게 올리면 무릎과 허리에 무리가 갈 수 있으므로 적당한 속도로 수축시켜줍니다.",
        videos: [
            { 
                title: "리버스 하이퍼(백) 익스텐션 머신 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/6I0NiRc6yww/0.jpg", 
                videoLink: "https://youtu.be/6I0NiRc6yww?si=KZFNonABRP2XEMWa"
            },
            { 
                title: "리버스 하이퍼(백) 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/kndkQhbvgTU/0.jpg", 
                videoLink: "https://youtu.be/kndkQhbvgTU?si=nyB37vrbz_NVnZX3&t=30"
            },
            { 
                title: "리버스 하이퍼(백) 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/jUSo_NUnpzo/0.jpg", 
                videoLink: "https://youtu.be/jUSo_NUnpzo?si=YewWCm8VS-lfuyNP&t=919"
            },
            { 
                title: "리버스 하이퍼(백) 익스텐션 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/fJtfxLTyml0/0.jpg", 
                videoLink: "https://youtube.com/shorts/fJtfxLTyml0?si=_cMBPIZdM-xRqD3K"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/reverse-hyper-extension.jpeg"
    },
    { 
        displayName: "치닝디핑(치닝)",
        name: "chinning-dipping1",
        muscle: "상체",
        description: "딥스와 턱걸이를 결합한 운동으로,\n딥스는 가슴과 이두근을 집중적으로 자극하는 운동입니다.\n턱걸이는 광배, 등 상부, 이두근을 발달시키는 데 효과적인 운동입니다.",
        exerciseInstructions: "1. 친 업 바 아래에 서서 얼굴이 바를 향하도록 합니다.\n2. 어깨 너비보다 살짝 넓게 양손으로 친 업 바를 잡아줍니다.\n3. 호흡을 들이마시며 가슴이 바에 닿을 때까지 팔꿈치를 굽혀 체중을 들어 올립니다.\n4. 호흡을 내쉬며 팔꿈치가 완전히 펴지기 전까지만 내려옵니다.",
        warning: "1. 과도하게 아래로 내려가거나 위로 올라가지 않습니다.\n2. 팔을 완전히 펴지 않도록 주의합니다.",
        videos: [
            { 
                title: "치닝디핑(치닝) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/VhfJB-VQOfg/0.jpg", 
                videoLink: "https://youtu.be/VhfJB-VQOfg?si=4v9Bn_BRRHKuXCni"
            },
            { 
                title: "치닝디핑(치닝) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/6hNsgWcSOJk/0.jpg", 
                videoLink: "https://youtu.be/6hNsgWcSOJk?si=TBeSRizcu13PdvFa"
            },
            { 
                title: "치닝디핑(치닝) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/6WeRstM2c0w/0.jpg", 
                videoLink: "https://youtube.com/shorts/6WeRstM2c0w?si=YN_LX8rE5s6LpmKl"
            },
            { 
                title: "치닝디핑(치닝) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/T-EZHtEx8Jw/0.jpg", 
                videoLink: "https://youtube.com/shorts/T-EZHtEx8Jw?si=ZEmZR1aFsimt-iHs"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/chinning-dipping1.jpeg"
    },
    { 
        displayName: "치닝디핑(디핑)",
        name: "chinning-dipping2",
        muscle: "상체",
        description: "딥스와 턱걸이를 결합한 운동으로,\n딥스는 가슴과 이두근을 집중적으로 자극하는 운동입니다.\n턱걸이는 광배, 등 상부, 이두근을 발달시키는 데 효과적인 운동입니다.",
        exerciseInstructions: "1. 양손으로 바를 잡고 몸을 앞으로 살짝 기울여 줍니다.\n2. 팔꿈치를 편 상태에서 무릎을 접어 몸을 공중에 띄웁니다.\n3. 팔을 몸 옆에 가까이 붙인 상태에서 팔꿈치의 각도가 90도가 될 때까지 굽혀주며 서서히 몸을 내려갑니다.\n4. 호흡을 내쉬면서 몸을 올라갑니다.",
        warning: "1. 딥스는 그립을 강하게 잡을수록 동작이 편해집니다.\n2. 평행봉을 잡고 등을 조여서 어깨를 아래로 내린다는 느낌으로 준비 자세를 취합니다.\n3. 내리는 동작에서 몸을 너무 내리면 가슴으로 가는 효과가 줄어들 수 있으므로 가슴의 이완을 느낄 정도로만 내려가갑니다.\n4. 팔을 몸통에 완전히 붙인 상태로 팔꿈치가 완전히 펴지기 전까지만 내려갑니다.\n5. 올라갈 때 가슴으로 바닥을 민다는 느낌으로 수축하며 올라갑니다, 의도적으로 삼각근(팔)의 개입을 최대한 줄여주어야 합니다.\n6. 딥스는 본인의 체중을 컨트롤할 수 있을 만큼의 근력과 체력이 안 될 경우 동작하기 어렵습니다, 초보자들은 어시스트 딥스 머신이나 벤치 딥스 머신을 사용하는 것을 권장합니다.",
        videos: [
            { 
                title: "치닝디핑(디핑) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/CbhjAZmrcfw/0.jpg", 
                videoLink: "https://youtu.be/CbhjAZmrcfw?si=bmfBh1f5xocjrRM-"
            },
            { 
                title: "치닝디핑(디핑) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/pqn9koa_wrw/0.jpg", 
                videoLink: "https://youtu.be/pqn9koa_wrw?si=o2hU0JdogD6qkhrG"
            },
            { 
                title: "치닝디핑(디핑) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/BC5tLHbAf78/0.jpg", 
                videoLink: "https://youtube.com/shorts/BC5tLHbAf78?si=HvZcAvlWeRIK4JAH"
            },
            { 
                title: "치닝디핑(디핑) 기본 운동법", 
                thumbnail: "https://img.youtube.com/vi/YxNDfXvwI08/0.jpg", 
                videoLink: "https://youtube.com/shorts/YxNDfXvwI08?si=YJ5BS5qsJp6TVCOL"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/chinning_dipping2.jpg"
    },
    { 
        displayName: "레그 프레스",
        name: "leg-press",
        muscle: "하체",
        description: "시티드 레그 프레스는 하체를 안정적으로 단련하는 데 효과적이며,\n특히 대퇴사두근과 하복부 근육을 강화하는 데 효과적입니다.\n무릎과 고관절에 직접적인 부담을 줄일 수 있는 자세의 하체 운동입니다.",
        exerciseInstructions: "1. 몸을 머신 중앙에 위치시키고, 발판 중앙에 발을 올려줍니다.\n2. 다리는 어꺠너비로 벌려주시고, 발끝은 30도정도로 바깥을 보게 합니다.\n3. 가슴은 펴고, 허리는 살짝 아치 형태로 복부에 긴장감을 줍니다.\n4. 호흡을 들이마시며 허벅지와 몸통이 수직이 될 때까지 굽혀줍니다.\n5. 호흡을 내쉬며 무릎이 다 펴지기 전까지만 밀어줍니다.",
        warning:"1. 발판 중앙보다 위쪽에 발을 위치시키면 허벅지 뒷쪽에 자극을 줄 수 있으며, 아래쪽에 발을 위치시키면 허벅지 앞쪽에 자극을 줄 수 있습니다.\n2. 무릎이 완전히 펴지지 않도록 해야합니다.\n3. 무릎이 벌어지면 안됩니다.",
        videos: [
            { 
                title: "시티드 레그 프레스 기본 운동법 1", 
                thumbnail: "https://img.youtube.com/vi/UokTSaDsk_A/0.jpg", 
                videoLink: "https://youtu.be/UokTSaDsk_A?si=bkgedGJ30L7Ns4Tb"
            },
            { 
                title: "시티드 레그 프레스 기본 운동법 2", 
                thumbnail: "https://img.youtube.com/vi/eK_4Mm7lAb0/0.jpg", 
                videoLink: "https://youtube.com/shorts/eK_4Mm7lAb0?si=bQbB1gdD6MWb94Vx"
            },
            { 
                title: "시티드 레그 프레스 기본 운동법 3", 
                thumbnail: "https://img.youtube.com/vi/srwB2HUrVAM/0.jpg", 
                videoLink: "https://youtube.com/shorts/srwB2HUrVAM?si=BaMghI8BqBavszup"
            },
            { 
                title: "시티드 레그 프레스 기본 운동법 4", 
                thumbnail: "https://img.youtube.com/vi/6k7fBUE3rec/0.jpg", 
                videoLink: "https://youtube.com/shorts/6k7fBUE3rec?si=wlGun9rigHq8Q4_p"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/leg-press.jpeg"
    },
    { 
        displayName: "레그 익스텐션",
        name: "leg-extension",
        muscle: "하체",
        description: "레그 익스텐션은 허벅지 앞쪽 근육(대퇴사두근)을 대상으로 한 하체 운동으로\n허벅지의 두꼐, 볼륨을 살릴 수 있는 운동입니다.\n무릎의 반동으로 해당 운동을 수행하면 관절에 무리가 갈 수 있으므로\n어느 부위에 힘이 들어가야 하는지 잘 알고 있는 상태에서 수행해야 합니다.",
        exerciseInstructions: "1. 엉덩이를 최대한 등받이에 가깝게 붙여주고 무릎 뒷쪽이 안장 패드 끝부분에 닿도록 앉아줍니다.\n2. 발목 패드를 발목보다 윗쪽에 닿도록 조절해줍니다.\n3. 엉덩이와 등을 머신에 밀착한 상태로 숨을 내쉬며 무릎이 완전히 펴지기 전까지만 다리를 올려줍니다.\n4. 호흡을 마시며 무릎이 90도가 될 때까지 다리를 내려줍니다.",
        warning:"1. 다리를 올릴 때 무릎이 아닌 허벅지에 힘이 가도록 허벅지 앞쪽 근육을 수축합니다.\n2. 절대 무릎의 반동을 이용해 다리를 들어올리지 않습니다, 관절에 무리가 갈 수 있으므로 자신이 올바른 자세로 수행할 수 있는 횟수만큼만 진행합니다.\n3. 엉덩이와 등을 등받이에 최대한 밀착해야 하며 뜨지 않도록 주의합니다.",
        videos: [
            { 
                title: "레그 익스텐션 기본 사용법1", 
                thumbnail: "https://img.youtube.com/vi/6P62nYjEMR8/0.jpg", 
                videoLink: "https://youtu.be/6P62nYjEMR8?si=mLFz4_kE9jL_ICqi"
            },
            { 
                title: "레그 익스텐션 기본 사용법2", 
                thumbnail: "https://img.youtube.com/vi/mS9iwXhycJI/0.jpg", 
                videoLink: "https://youtu.be/mS9iwXhycJI?si=CNEjl8KK2ipmXDOc"
            },
            { 
                title: "레그 익스텐션 기본 사용법3", 
                thumbnail: "https://img.youtube.com/vi/XnnzPScNTYk/0.jpg", 
                videoLink: "https://youtube.com/shorts/XnnzPScNTYk?si=bkaZSF07ZrNQHxQZ"
            },
            { 
                title: "레그 익스텐션 기본 사용법4", 
                thumbnail: "https://img.youtube.com/vi/TKXkq1JiQ0o/0.jpg", 
                videoLink: "https://youtube.com/shorts/TKXkq1JiQ0o?si=ZCTG5DuyxwUmALpI"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/leg_extension.jpg"
    },
    { 
        displayName: "이너 타이",
        name: "inner-thigh",
        muscle: "하체",
        description: "허벅지 안쪽 내전근을 강화시키는 운동으로\n허벅지 안쪽살과 골반의 안정화에 효과적인 운동입니다.\n이너 타이는 힙어덕션으로 많이 알려져 있으며,\n힙의 탄력 강화와 허벅지 안쪽살에 자극이 뛰어나\n남성보다는 여성이 주로 즐겨합니다.",
        exerciseInstructions: "1. 허리를 곧게 세워 엉덩이와 등을 등받이에 밀착시켜 앉습니다.\n2. 손잡이를 잡고 발은 발판 위에 놓은 상태로, 패드가 무릎 위의 허벅지 안쪽에 닿도록 조정합니다.\n3. 조절핀을 사용하여 허벅지가 최대한 멀리 벌어질 수 있도록 자신의 유연성을 고려하여 조절합니다.\n4. 손잡이를 잡은 상태에서 허벅지에 힘을 줘 양쪽 다리를 모아줍니다.\n5. 호흡을 들이마시며 저항을 느끼면서 서서히 다리를 벌려줍니다.",
        warning:"1. 엉덩이를 등받이에 밀착시켜준 상태로 운동을 수행해야 합니다.\n2. 가동 범위가 작으면 운동 효과를 볼 수 없으므로 허벅지에 자극이 오는 위치까지 벌릴 수 있도록 고정핀의 위치를 조절해야 합니다.\n3. 다리를 모을 때 너무 빨리 모으지 않습니다, 최대한 저항을 느끼며 천천히 이완해야 합니다.\n4. 과도한 중량보다는 근육이 늘어나는 느낌에 집중하여 운동해야 합니다.",
        videos: [
            { 
                title: "이너타이 기본 사용법1", 
                thumbnail: "https://img.youtube.com/vi/MO6u_EEjjyk/0.jpg", 
                videoLink: "https://youtu.be/MO6u_EEjjyk?si=Z-rQRff7lE7eWkp4&t=319"
            },
            { 
                title: "이너타이 기본 사용법2", 
                thumbnail: "https://img.youtube.com/vi/JbKtKZwlGVA/0.jpg", 
                videoLink: "https://youtu.be/JbKtKZwlGVA?si=me2nI5SVEtCmNtLJ"
            },
            { 
                title: "이너타이 기본 사용법3", 
                thumbnail: "https://img.youtube.com/vi/0ASD0EZ7l_Q/0.jpg", 
                videoLink: "https://youtu.be/0ASD0EZ7l_Q?si=3Y9UKhwEXLNNWYCB&t=184"
            },
            { 
                title: "이너타이 기본 사용법4", 
                thumbnail: "https://img.youtube.com/vi/IIx2bCMPGDw/0.jpg", 
                videoLink: "https://youtube.com/shorts/IIx2bCMPGDw?si=dXV1kARP5-BZuhbD"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/inner_thigh.jpg"
    },
    { 
        displayName: "아웃 타이",
        name: "out-thigh",
        muscle: "하체",
        description: "이너 타이는 다리를 모아주는 동작인 반면,\n아웃 타이는 다리를 벌려주는 운동입니다.\n대퇴골 바깥에 위치한 근육인 중둔근 발달에 효과적인 운동으로,\n주로 다리를 옆으로 들거나 움직일 때 사용됩니다.\n때문에 이 근육이 발달하지 않으면\n힙 딥(hip-dip) 현상이 나타나며 고관절이 불안정해집니다.\n힙 어브덕션으로 많이 알려져 있으며, 남성보다는 여성이 주로 즐겨하는 운동 중 하나입니다.",
        exerciseInstructions: "1. 허리를 편 채로 골반을 뒤로 빼 엉덩이를 등받이에 밀착시킵니다.\n2. 패드를 허벅지 바깥쪽에 위치시킨 후, 손잡이를 잡아줍니다.\n3. 허벅지 바깥쪽에 자극을 느끼며 다리를 벌려줍니다.\n4. 호흡을 들이쉬며 긴장을 유지한 채로 허벅지를 천천히 모아줍니다.",
        warning:"1. 상체를 살짝 기울여 고관절을 접어준 상태에서 시작하거나, 허리를 세운 상태에서 시작하는 등 추천해주는 자세가 추천해주는 사람마다 다를 수 있습니다, 자신이 편한 자세로 하면 됩니다.\n2. 다리를 모을 때 빠르게 모아주는 것이 아닌 저항을 느끼며 천천히 이완시켜줍니다.\n3. 이너 타이보다 중량을 높여서 하는 것을 추천드립니다.",
        videos: [
            { 
                title: "아웃타이 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/D_HUq5odZhg/0.jpg", 
                videoLink: "https://youtu.be/D_HUq5odZhg?si=ZbLUvBT9qpuF_OZC"
            },
            { 
                title: "아웃타이 기본 운동법2", 
                thumbnail: "https://img.youtube.com/vi/nwQcyG3uYDI/0.jpg", 
                videoLink: "https://youtu.be/nwQcyG3uYDI?si=6zgGPNIoqVertY0E"
            },
            { 
                title: "아웃타이 기본 운동법3", 
                thumbnail: "https://img.youtube.com/vi/SwtIc_lKeGY/0.jpg", 
                videoLink: "https://youtube.com/shorts/SwtIc_lKeGY?si=c6tQhJyG5oz79r5t"
            },
            { 
                title: "아웃타이 기본 운동법4", 
                thumbnail: "https://img.youtube.com/vi/Ry2vAFCzqVQ/0.jpg", 
                videoLink: "https://youtube.com/shorts/Ry2vAFCzqVQ?si=h9VmrscI0HYfHbpy"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/out_thigh.jpg"
    },
    { 
        displayName: "런닝머신",
        name: "treadmill",
        muscle: "유산소",
        description: "가장 대중적인 유산소 운동 기구로 심폐 지구력 향상과 체지방 감량에 효과적입니다.\n다양한 속도와 경사 조절 기능을 통해 맞춤형 워크아웃이 가능합니다.",
        exerciseInstructions: "1. 자신에게 맞는 적당한 속도로 설정합니다.\n2. 시선을 앞을 향하도록 하고, 어깨의 힘을 뺀 상태로 팔을 앞뒤로 흔들면서 발 뒷꿈치부터 닿도록 걷습니다.\n3. 숨을 들어마시는 것은 코로 하며 내쉬는 것은 입으로 합니다.",
        warning:"1. 전용 런닝화를 신는 것이 좋습니다.\n2. 처음부터 빠른 속도로 두는 것이 아니라 천천히 늘려갑니다.\n3. 유산소 운동을 하기 전 햄스트링을 충분히 스트레칭 해주는 것이 좋습니다.",
        videos: [
            { 
                title: "런닝머신 기본 운동법 1", 
                thumbnail: "https://img.youtube.com/vi/7eku-lsS0AM/0.jpg", 
                videoLink: "https://youtu.be/7eku-lsS0AM?si=lPMDLKFvUGlUgNhD"
            },
            { 
                title: "런닝머신 기본 운동법 2", 
                thumbnail: "https://img.youtube.com/vi/d8Jm_6TZxL0/0.jpg", 
                videoLink: "https://youtube.com/shorts/d8Jm_6TZxL0?si=UKqS_YSDYFG74_My"
            },
            { 
                title: "런닝머신 기본 운동법 3", 
                thumbnail: "https://img.youtube.com/vi/lCS5rXtm6qk/0.jpg", 
                videoLink: "https://youtube.com/shorts/lCS5rXtm6qk?si=hkoOzL0PZ_ZGlwww"
            },
            { 
                title: "런닝머신 기본 운동법 4", 
                thumbnail: "https://img.youtube.com/vi/Kk-IConBOf0/0.jpg", 
                videoLink: "https://youtube.com/shorts/Kk-IConBOf0?si=zx08KbGax8FROJjG"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/treadmill.jpeg"
    },
    { 
        displayName: "스텝퍼",
        name: "stepper",
        muscle: "유산소",
        description: "유산소 운동 중 시간 대비 칼로리 소모가 많은 운동으로,\n실제 계단을 올라가는 듯한 움직임을 갖고 있는 운동입니다.\n심폐 지구력을 향상시키고 하체 근력을 강화시켜줍니다.",
        exerciseInstructions: "1. 적당한 속도로 설정한 뒤 상체를 살짝 앞으로 기울인 자세를 취합니다.\n2. 발판을 천천히 뒷꿈치로 누르며 양쪽 발을 번갈아갑니다.",
        warning: "1. 계단을 오를 때 발의 무게중심을 앞쪽에 두면 운동 효과가 떨어지기 때문에 뒷꿈치가 발판에 먼저 닿도록 올라가야 합니다.\n2. 상체를 일직선으로 펴면 허리가 무리가 갈 수 있으니 살짝 기울인 자세를 유지해야 합니다.",
        videos: [
            { 
                title: "스텝퍼 운동 효과1", 
                thumbnail: "https://img.youtube.com/vi/cbaDfVxDMig/0.jpg", 
                videoLink: "https://youtu.be/cbaDfVxDMig?si=6hL72lIjboyPHhGY&t=256"
            },
            {
                title: "스텝퍼 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/GscAz382IBw/0.jpg", 
                videoLink: "https://youtube.com/shorts/GscAz382IBw?si=K2qrvaLWKh0mx-6i"
            },
            { 
                title: "스텝퍼 기본 운동법2", 
                thumbnail: "https://img.youtube.com/vi/RTM65wfz4D8/0.jpg", 
                videoLink: "https://youtube.com/shorts/RTM65wfz4D8?si=xj1B0shQLQ6DC9mU"
            },
            {
                title: "스텝퍼 기본 운동법3", 
                thumbnail: "https://img.youtube.com/vi/9Wc2ad17YW4/0.jpg", 
                videoLink: "https://youtube.com/shorts/9Wc2ad17YW4?si=N5LS4LDxTFYU9kvm"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/stepper.jpeg"
    },
    { 
        displayName: "일립티컬 머신",
        name: "elliptical-machine",
        muscle: "유산소",
        description: "일립티컬은 자전거처럼 페달을 밝으면서 걷는 것과 비슷한 움직임을 만들어내는 운동기구입니다.\n다리를 들어올리지 않고 앞뒤로 움직여 관절에 무리가 가지 않아 남녀노소 쉽게 접근할 수 있습니다.\n유산소 운동과 근력 운동을 동시에 할 수 있으며,\n스텝퍼와 더불어 유산소 운동 중 단시간에 큰 칼로리를 태울 수 있는 효과적인 운동기구입니다.",
        exerciseInstructions: "1. 손잡이를 잡고 두 발을 페달 위에 올려 똑바로 섭니다.\n2. 가슴은 펴고 허리와 머리를 똑바로 세운 상태로 페달을 밟는데, 이때 체중이 발바닥의 앞쪽으로 쏠리지 않도록 합니다.\n3. 낮은 강도와 좁은 보폭으로 시작하며 점차 강도를 높힙니다.\n4. 뒷꿈치를 붙이고 엉덩이를 살짝 뒤로 빼서 꼭 자전거를 타듯 엉덩이에 힘을 주면서 페달을 밀어줍니다.",
        warning:"1. 무릎의 방향은 발가락의 방향과 동일한 선상으로 놓아야 무릎에 무리가 적게 오며, 발을 팔자로 놓지 않도록 주의해야 합니다.\n2. 엉덩이를 과하게 뒤로 빼거나 상체를 앞으로 과하게 숙이지 않습니다.\n3. 운동을 하기 전 충분히 상하체 스트레칭을 진행합니다.",
        videos: [
            { 
                title: "일립티컬 머신 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/QIZWdCZTxN4/0.jpg", 
                videoLink: "https://youtu.be/QIZWdCZTxN4?si=KxnkJDNerPWdw82-"
            },
            { 
                title: "일립티컬 머신 기본 운동법2", 
                thumbnail: "https://img.youtube.com/vi/q3_2-Pri7xA/0.jpg", 
                videoLink: "https://youtu.be/q3_2-Pri7xA?si=tz_XKANAihTaEYij"
            },
            { 
                title: "일립티컬 머신 기본 운동법3", 
                thumbnail: "https://img.youtube.com/vi/hm0tz88LSww/0.jpg", 
                videoLink: "https://youtu.be/hm0tz88LSww?si=dZOiCdOW0VWcFwBS"
            },
            { 
                title: "일립티컬 머신 기본 운동법4", 
                thumbnail: "https://img.youtube.com/vi/C8YKyoUXBPo/0.jpg", 
                videoLink: "https://youtu.be/C8YKyoUXBPo?si=i7MzbiNyjyoiMClR"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/elliptical_machine.jpg"
    },
    { 
        displayName: "사이클",
        name: "cycle",
        muscle: "하체",
        description: "사이클은 런닝머신과 더불어 가장 대중적인 유산소 기구로,\n체중을 덜 실어 운동을 할 수 있으므로 걷기에 비해 무릎이나 허리에 부담이 덜 갑니다.\n때문에 과체중인 분들이 런닝머신에 비해 쉽게 운동할 수 있습니다.\n앉아서 수행하기에, 힘이 하지에 더욱 실리게 되어 걷는 것보다는 근육을 덜 사용하게 됩니다.",
        exerciseInstructions: "1. 자신이 기구 옆에 섰을 때 골반 라인에 위치할 수 있도록 안장의 높낮이를 조절합니다.\n2. 기구에 앉아 자신이 수행할 운동 강도에 맞게 페달 속도를 조절합니다.\n3. 페달에 발을 올리고 고정시킨 뒤, 전원 버튼을 눌러 자전거 타듯 움직여줍니다.",
        warning:"1. 페달에 발을 걸쳐놓는 것이 아닌 고정장치를 이용해 발이 페달 전체를 밟을 수 있도록 고졍시킵니다.\n2. 상체를 과하게 앞으로 숙이거나 허리가 과신전되지 않도록 주의합니다.",
        videos: [
            { 
                title: "사이클 효과1", 
                thumbnail: "https://img.youtube.com/vi/n7e_MGkS0f4/0.jpg", 
                videoLink: "https://youtu.be/n7e_MGkS0f4?si=F7y1NOpLLUL8xIBz"
            },
            { 
                title: "사이클 기본 운동법1", 
                thumbnail: "https://img.youtube.com/vi/sqC5a78NfQw/0.jpg", 
                videoLink: "https://youtu.be/sqC5a78NfQw?si=e735qaIRp--fZygT"
            },
            { 
                title: "사이클 효과2", 
                thumbnail: "https://img.youtube.com/vi/6XTDW8W5wsI/0.jpg", 
                videoLink: "https://youtube.com/shorts/6XTDW8W5wsI?si=Pg4HHXYhicG72GiM"
            },
            { 
                title: "사이클 효과3", 
                thumbnail: "https://img.youtube.com/vi/lrDPXvd2xCw/0.jpg", 
                videoLink: "https://youtube.com/shorts/lrDPXvd2xCw?si=tjjiAaQz5zhyVaY4"
            }
        ],
        images: process.env.PUBLIC_URL + "/images/cycle.jpg"
    }
];